...
\subsection{Row-Input Encoding Methodology}
\subsubsection{Input Encoding Methodology}
...
A significant innovation in the proposed methodology involves the encoding of patch information through amplitude-modulated carrier waves. For each patch vector $\vec{p}_i$, a time-varying voltage signal is generated using the following carrier wave generation scheme:
...
\subsubsection{Memristor State Evolution and Feature Extraction}

The encoded carrier waves serve as input stimuli to a bank of memristors, with each patch processed by a dedicated memristor selected through a cyclic assignment strategy:

\begin{equation}
    \text{memristor}_i = \text{memristors}[(i \bmod M) + 1]
\end{equation}

where $M$ represents the total number of available memristors in the reservoir architecture.

The temporal evolution of each memristor's state is governed by the MMS (Memristor Modeling System) differential equation:

\begin{equation}
    \frac{dx}{dt} = f_{\text{MMS}}(x, V_{\text{in}}(t), \theta)
\end{equation}

where $x \in [0,1]$ represents the normalized memristor state, $V_{\text{in}}(t)$ is the input voltage signal, and $\theta$ denotes the device-specific parameters.

To ensure computational accuracy while maintaining reasonable simulation times, the differential equation is solved using the Tsitkuras 5th order Runge-Kutta method (Tsit5), with adaptive time-stepping and explicit saving points aligned with the input signal sampling times:

\begin{equation}
    x(t) = \text{ODESolve}\left(f_{\text{MMS}}, x_0, [0, t_{\text{max}}], \text{Tsit5}\right)
\end{equation}

The solution trajectory captures the dynamic evolution of the memristor state in response to the modulated input signal. For computational efficiency, an Akima spline interpolation is employed to create a continuous representation of the discrete input signal:

\begin{equation}
    V_{\text{in}}^{\text{interp}}(t) = \text{AkimaInterp}(V_{\text{discrete}}, t_{\text{discrete}})
\end{equation}

This interpolation scheme preserves local monotonicity and avoids oscillatory artifacts that could adversely affect the memristor dynamics.
