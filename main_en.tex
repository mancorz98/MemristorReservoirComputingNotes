...
\section{Experimental Investigation of Memristor-Based Reservoir Computing}

The experimental investigation was conducted through computational simulations employing the Memristor Mean Metastable Switch (MMS) memristor model. The standardised MNIST (Modified National Institute of Standards and Technology) dataset, comprising 28$\times$28 pixel grayscale images of handwritten digits, was used as the classification benchmark. This dataset provides a well-established baseline for evaluating neuromorphic computing architectures and enables direct comparison with prior work.
...
Under the row-wise encoding, the reservoir system achieved a classification accuracy of 60.37\% on the MNIST test set. This relatively modest result reflects the inherent limitation of row-wise serialisation: each memristor processes a one-dimensional temporal signal devoid of local two-dimensional spatial context, constraining the representational capacity of the reservoir.

\subsection{Patch-Based Feature Extraction for Memristor Crossbar Array Reservoir}

To overcome the limitations of row-wise encoding and to exploit the parallel computational structure of memristor crossbar arrays, a patch-based spatial decomposition is applied to the input images prior to temporal encoding. This approach maps localised spatial features onto the row inputs of the crossbar, enabling simultaneous reservoir processing across all rows.
...
\subsubsection{Row-Wise Temporal Encoding for the Crossbar Array}
...
\begin{figure}[H]
    \centering
    \resizebox{0.5\textwidth}{!}{\input{tex_figs/workflow_diagram.tex}}
