...
\subsection{Patch Encoding for Memristor Crosbar Array Reservoir}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{vector_figs/Patch-algorithm.pdf}
    \caption{Patch-based feature extraction and memristor reservoir processing methodology for the memristor crossbar array reservoir computing system.}
    % \label{fig:memristor-rc}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Example of programming the first row of the memristor crossbar array reservoir with the patch-based feature extraction method.}
    % \label{fig:memristor-rc}
\end{figure}
...
For the provided patch-based feature extraction and memristor reservoir processing methodology, the memristor-based reservoir computing system achieved a classification accuracy of 80.25\% on the MNIST dataset (91.33\% for test datasets for the crossbar array). This performance metric underscores higher  efficacy of the proposed architecture in leveraging localized spatial features and the intrinsic dynamics of memristive devices for temporal information processing tasks. The significant improvement in classification accuracy compared to the row-wise processing approach can be attributed to the enhanced feature representation provided by the patch-based method, which captures local spatial correlations more effectively.

The experimental results demonstrate the potential of memristor-based reservoir computing systems for complex pattern recognition tasks, showing 87\% accuracy for the crossbar array reservoir. Future work will focus on further optimizing the patch extraction parameters, exploring alternative encoding schemes, and investigating the scalability of the architecture to larger datasets and more complex tasks.
