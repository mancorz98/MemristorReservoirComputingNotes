\begin{tikzpicture}[
    ...
    \node [crossbar] (core) at (0, -8.5) {\textbf{8x8 Memristive} \\ \textbf {Crossbar Array} \\   \begin{circuitikz}
    \draw (0,0) to[memristor] (2,0);
  \end{circuitikz}};
    ...
    \draw [line] (dac.south) -- node[left] {Analog $v_{ \rm in}$} (switch.north);
    \draw [line] (switch.south) -- node[left] {Analog $v_{ \rm in}$} (inbuf.north);
    \draw [line] (inbuf.east) -- node[above] {$v_{ \rm drive}$} (core.west);
    
    % Readout Path
    \draw [line] (core.east) -- node[above] {$i_{ \rm out}$} (sense.west);
    \draw [line] (sense.north) -- node[right] {Analog $v_{ \rm out}$} (lpf.south);
    \draw [line] (lpf.north) -- node[right] {Filtered $v_{ \rm \mathrm{out}}$} (outbuf.south); % Changed to right and "Filtered" for clarity
